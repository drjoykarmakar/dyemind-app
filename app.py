...
def query_huggingface(payload):
    ...
    """
    # We use Mistral-7B-Instruct-v0.2 because it is fast, free, and smart.
...
def get_pubchem_data(query):
    ...
    try:
        ...
        search = requests.get(f"{base_url}/compound/name/{query}/cids/JSON").json()
        cid = search['IdentifierList']['CID'][0]
        
        props = requests.get(f"{base_url}/compound/cid/{cid}/property/CanonicalSMILES,MolecularFormula,MolecularWeight/JSON").json()
...
def get_pubmed_literature(query):
    ...
    try:
        ...
        search = requests.get(f"{db_url}/esearch.fcgi?db=pubmed&term={term}&retmode=json&retmax=5&sort=relevance").json()
...
if query:
    ...
    if chem:
        with col1:
            st.image(chem['image'], caption=f"Structure (CID: {chem['cid']})", use_container_width=True)
    ...
    with col2:
        if chem or articles:
            ...
            # Download Button
            st.download_button("📥 Download Report", review, file_name=f"{query}_report.md")
        else:
